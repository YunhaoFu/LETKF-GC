...
module kdtree
  ...
  use precision  ! todo_modified by fuyunhao
  implicit none
  private
...
contains
  ...
  subroutine kd_init(root, lons, lats)
    ...
    real(r4), intent(in), target :: lats(:)
    !! list of latitudes (degrees)

    real(r4), intent(in), target :: lons(:)
    ...
    do n=1, size(lons)
       ...
       root%pts(n,:) = ll2xyz( lons(n)*1.0_r8, lats(n)*1.0_r8 )
    end do
  ...
  subroutine kd_search_radius(root,  s_lon, s_lat, s_radius, r_points, r_distance, r_num, exact)
    ...
    ! local variables
    real(r8) :: s_xyz(kd_dim)
    real(r8) :: r, slatr, clatr, lonr
    ...
    s_xyz = ll2xyz(s_lon*1.0_r8, s_lat*1.0_r8)
  ...
  pure function ll2xyz(lon, lat)
    ...
    ll2xyz(1) = re * cos(lat*pi/180.0_r8) * cos(lon*pi/180.0_r8)
    ll2xyz(2) = re * cos(lat*pi/180.0_r8) * sin(lon*pi/180.0_r8)
    ll2xyz(3) = re * sin(lat*pi/180.0_r8)
  ...
  pure function dist_gc(lon1, clat1, slat1, lon2, lat2)
    ...
    real(r8), intent(in) :: lon1, clat1, slat1
    !! point 1 longitude (degrees), cosine of latitude, sine of latitude

    real(r8), intent(in) :: lon2, lat2
